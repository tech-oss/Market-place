...
const TXN_TONE = {
  sale: "green", commission: "gray", payout: "blue", refund: "red",
} as const;

export default async function SellerWalletPage() {
  const [{ balanceCents, pendingCents, txns }, pct] = await Promise.all([
    getWallet(),
    getCommissionPct(),
  ]);
              ...
              <tbody className="divide-y divide-border">
                ...
                {txns.map((t) => (
                  <tr key={t.id}>
                    ...
                    <td className="px-5 py-3"><StatusPill label={t.type} tone={TXN_TONE[t.type]} /></td>
                    <td className="px-5 py-3"><StatusPill label={t.status} tone={t.status === "completed" ? "green" : "amber"} /></td>

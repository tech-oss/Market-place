...
import { sellerOrders as seed } from "@/mocks/dashboard";
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      >
        ...
        <button type="submit" className="w-full rounded-lg bg-brand px-4 py-2.5 text-sm font-semibold text-brand-foreground hover:opacity-90">
          Confirm shipment
        </button>
      </form>
    </div>
...
export default function SellerOrdersPage() {
  const [orders, setOrders] = useState<SellerOrder[]>(seed);
    ...
    <>
                    ...
                    <td className="px-5 py-3 text-right">
                      {o.status === "paid-held" ? (
                        <button
                          ...
                          className="inline-flex items-center gap-1.5 rounded-lg bg-ink px-3 py-1.5 text-xs font-semibold text-white hover:bg-neutral-800"
                        >
      ...
      {shipping && (
        <ShipDialog
          ...
          onShip={(courier, tracking) => {
            setOrders((prev) =>
              prev.map((o) => (o.id === shipping.id ? { ...o, status: "shipped", courier, tracking } : o)),
            );
            setShipping(null);
          }}
        />
